...
const LeftKey = ({ item, onPress }: { item: string; onPress: () => void }) => {
  return (
    <Pressable
      className="flex h-20 w-[30%] items-center justify-center rounded-xl bg-input/30 px-4 py-2"
      onPress={onPress}>
      <Text
...
const RightKey = ({
  ...
  return (
    <Pressable
      className="flex h-20 w-full items-center justify-center rounded-xl bg-input/30 px-4 py-2"
      onPress={onPress}>
      <Text
        className="text-sm font-semibold text-foreground"
        style={{
...
const CustomKeyboards = ({
  ...
  return (
    <View className="mt-2 flex flex-row items-center gap-1 px-2">
      ...
      <View className="flex w-[25%] flex-col gap-2">
        <RightKey item="Back" icon="close-circle-outline" onPress={onBackspace} />
        <RightKey item="Back" icon="trash-outline" onPress={onClear} />
        <Pressable
          className="flex h-40 w-full items-center justify-center rounded-xl bg-foreground px-4 py-2"
          onPress={onConfirm}>
          <Text
            className="text-sm font-semibold text-foreground"
            style={{
